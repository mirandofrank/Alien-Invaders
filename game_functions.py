...
def check_keydown_events(event, ai_settings, screen, ship, bullets):  # Key is being pushed
...
def check_keyup_events(event, ship):  # Key is released
    if event.key == pygame.K_RIGHT:
        ship.moving_right = False  # Done moving right
    elif event.key == pygame.K_LEFT:
        ship.moving_left = False  # Done moving left
...
def check_events(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
        elif event.type == pygame.KEYDOWN:  # When key is pressed down
